...
class LocationModule {
    ...
    showPossibleOptions() {
        try {
            for ( let i = 0; i < 3; i++ ) {
                const rand = Math.floor(Math.random() * (this.locationList.length - 0))
                this.pickList.innerHTML += `
                    <li class="pick-address__item" data-id="${this.locationList[rand]._id}">
                        ${this.locationList[rand].name}
                    </li>`
            }
            this.registerHandlersForPickItems()
        } catch (e) {
            this.pickList.innerHTML += `
                    <li class="pick-address__item pick-address__item_notfound">
                       Ничего не найдено
                    </li>`
        }
    }
